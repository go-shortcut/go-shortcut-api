...
func main() {
	...
	if len(os.Args) > 1 {
		...
		stories, err := shortcutClient.ListGroupStories(firstCmdArg)
		if err != nil {
			panic(err)
		}
		...
		for _, s := range stories {
			if !s.Completed && !s.Archived {
				...
				fmt.Printf("%s\t%s\t%s\t%s\n", s.AppURL, getWorkFlowState[s.WorkflowStateID].Name, owners, strings.Replace(s.Name, "\n", "", -1))
		...
		groups, err := shortcutClient.ListGroups()
		if err != nil {
			panic(err)
		}
